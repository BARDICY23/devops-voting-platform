...
echo "Waiting for DB..."
until PGPASSWORD=$POSTGRES_PASSWORD pg_isready -h voting-app-postgresql -U postgres -d postgres >/dev/null 2>&1; do
    echo "DB not ready, retrying..."
    sleep 1
done

echo "Creating table if not exists..."
PGPASSWORD=$POSTGRES_PASSWORD psql -h voting-app-postgresql -U postgres -d postgres \
